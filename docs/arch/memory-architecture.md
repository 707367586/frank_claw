...
长期记忆                         → Long-Term Memory
  ...
  └── 程序记忆 (技能/习惯)       →   Agent Memory, kind='skill' (学到的工作模式)
```

> **说明：** 人类的"程序记忆"在 ClawX 中作为 Agent Memory 的 `kind='skill'` 类型实现，而非独立的存储层。这是因为技能/习惯与 Agent 的任务经历紧密关联，共享同一套衰减、召回和审计机制更为合理。
...
- 隐式提取暂停，待 LLM 恢复后不补提（已过去的对话轮次不回溯）
- 显式提取（用户明确说"记住 XXX"）：通过信号词规则检测直接存储原文，`source_type='explicit'`，不经 LLM 结构化处理
- 记忆召回不受影响（纯检索，不依赖 LLM）
- LLM 恢复后自动恢复正常提取流程，无需人工干预
